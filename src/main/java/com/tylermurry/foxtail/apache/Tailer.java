/*
This is a direct copy of org.apache.commons.io.input.Tailer.

The only change that had to be made was to ignore file reloads when the file was newer.
This was causing it to reload the entire file into the log again causing spamming issues.
 ...
 * <p>
 * <pre>
 *      TailerListener tailer = new MyTailerListener();
 *      Tailer service = Tailer.create(file, tailer, delay);</pre>
 * <p>
 * <h3>2.2 Using an Executor</h3>
 * <p>
 * <pre>
 *      TailerListener tailer = new MyTailerListener();
 *      Tailer service = new Tailer(file, tailer, delay);
 *
 ...
 * <p>Remember to stop the service when you have done with it:</p>
 * <pre>
 *      service.stop();
 * </pre>
 * <p>
 * <h2>4. Interrupting a Tailer</h2>
 * <p>You can interrupt the thread a service is running on by calling {@link Thread#interrupt()}.</p>
 * <pre>
 *      thread.interrupt();
 * </pre>
 * <p>If you interrupt a service, the service tailer is called with the {@link InterruptedException}.</p>
 * <p>
...
public class Tailer implements Runnable
{
	...
	/**
	 * The tailer to notify of events when tailing.
	 */
	...
	/**
	 * The service will run as long as this value is true.
	 */
		...
		try
		{
			...
			while (getRun())
			{
				...
				else
				{
					...
					{
						// This section was causing the service to reload the entire log into the file again
						// which was spamming it. Doing nothing instead.
					}
				}
				if (reOpen)
				{
	...
	/**
	 * Stops the service with an exception
	 *
	 * @param e The exception to send to tailer
	 */
	...
	/**
	 * Allows the service to complete its current loop and return.
	 */
	public void stop()
	{
		this.run = false;
